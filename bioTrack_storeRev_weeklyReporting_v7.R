...
# v5 removes dupes in a dumb way, but it works-ish.  The total revenue doesn't match biotrack exactly because of the dupe trim.
...
# need to convert histDat$inventory id from factor to numeric by getting rid of spaces
...
# get license number from  using manifestid in manifest_stop_data, which you can get from inventoryid

# connect to PH_staging (aka Biotrack API)
channel <- odbcConnect("PH_staging", uid="will.wright", pwd="t8032Q5J&8w5") # connect by entering credentials

# grab manifest stop data
...
# 8005 rows, so 8 more, but revenue is stil 2051884 so at least that doens't blow up
...
########################################################################################################################
# APPEND LAST UPDATED BY USING LEAFLY MENU AND LCBID
########################################################################################################################
# just FYI, after I did this whole thing, I found that the last updated field didn't help much.  All the places we've sold
# to are still in business and it's faster just to google em real quick than to rely on last updated.
...
lcbLookup <- read.csv("stores_leafly_502_lookup_v6.csv")
menuLcb <- merge(menu, lcbLookup[,c(1,2,4)], by.x = "dispensary_id", by.y = "leafly_id")
